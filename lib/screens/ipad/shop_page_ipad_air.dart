...
class ShopPageIpadAir extends StatelessWidget {
  const ShopPageIpadAir({super.key});

  @override
  Widget build(BuildContext context) {
    final ipadShop = Provider.of<IpadShop>(context);
    final airIpads = ipadShop.getIpadsByCategory('ipad air');
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView.builder(
          itemCount: airIpads.length,
          itemBuilder: (context, index) {
            return Padding(
              padding: const EdgeInsets.only(bottom: 16.0),
              child: IpadProductTile(ipad: airIpads[index]),
            );
          },
        ),
      ),
